...
document.addEventListener("DOMContentLoaded", function () {
	...
	function documentActions(e) {
		...
		if (targetElement.closest(".blogs__category-item") && document.querySelector(".example")) {
			...
			if (link && link.getAttribute("href")?.startsWith("#")) {
				...
				if (scrollTarget) {
					...
					const header = document.querySelector(".header");
					const headerOffset = header ? header.offsetHeight + blogsFixed.offsetHeight : 0;

					const elementPosition = scrollTarget.getBoundingClientRect().top;
					const offsetPosition = elementPosition - 55 + window.scrollY - headerOffset;
...
animationsConfig.forEach(({ selector, keyframes, options, nextDelay }) => {
	document.querySelectorAll(selector).forEach((el) =>
		animateOnScroll(el, keyframes, options, nextDelay)
	);
});
